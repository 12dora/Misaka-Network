...
const { issueCredentials, banSession, getTurnStatus } = await import('../dist/turn.js')
...
async function main() {
  ...
  console.log('[2] Pre-existing session ban → SESSION_BANNED')
  banSession('sess-banned', 'manual-test')
  const r2 = await issueCredentials('sess-banned', '9.9.9.9')
  assert(!r2.ok && r2.reason === 'SESSION_BANNED', 'banned session rejected')

  console.log('[3] Per-IP byte cap (pessimistic)')
  // IP 3.3.3.3: 1 issuance adds 30 MB (under 40 MB cap), 2nd would push to 60 MB
  const r3a = await issueCredentials('sess-B', '3.3.3.3')
  assert(r3a.ok === true, 'first issue under cap succeeds')
  const r3b = await issueCredentials('sess-C', '3.3.3.3')
  assert(!r3b.ok && r3b.reason === 'IP_BYTES_LIMITED', 'IP_BYTES_LIMITED on 2nd cred over cap')

  console.log('[4] Per-IP issuance rate cap')
  ...
  const r4 = await issueCredentials('sess-D', ip)
  assert(!r4.ok && r4.reason === 'IP_RATE_LIMITED', 'IP_RATE_LIMITED at cap')

  console.log('[5] Global kill switch')
  ...
  const r5 = await issueCredentials('sess-E', '5.5.5.5')
  assert(!r5.ok && r5.reason === 'GLOBAL_QUOTA_EXCEEDED', 'GLOBAL_QUOTA_EXCEEDED while kill switch active')
  assert(getTurnState().monthlyUsage.killSwitchActive, 'killSwitchActive flag set')

  console.log('[6] Status endpoint payload (no secrets)')
  ...
  console.log('[7] Persistence: flush + retain deny list / month tally')
  ...
  assert(persisted.denyList.sessions['sess-banned'], 'banned session persisted')
